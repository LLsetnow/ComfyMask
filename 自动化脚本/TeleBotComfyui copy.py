        ...
        try:
            ...
            for update in updates["result"]:
                ...
                if "photo" in message:
                    ...
                    # 下载图片
                    send_message(chat_id, f"收到图片，正在使用「{DEFAULT_WORKFLOW}」处理...")
                    print("  开始下载图片...")
                    ...
                    if image_path:
                        ...
                        try:
                            ...
                            thread = threading.Thread(
                                ...
                                args=(image_path, chat_id, DEFAULT_WORKFLOW)
                            )
                            thread.daemon = True
                            thread.start()
                        except Exception as e:
                ...
                # 处理文档消息（可能是图片文件）
                    ...
                    if mime_type and mime_type.startswith("image/"):
                        ...
                        send_message(chat_id, f"收到图片文档，正在使用「{DEFAULT_WORKFLOW}」处理...")
                        image_path = download_telegram_photo(file_id)
                ...
                # 处理文本消息
                    ...
                    if text == "/start":
                        send_message(chat_id,
                                   ...
                                   f"发送图片给我，将使用「{DEFAULT_WORKFLOW}」处理方式。\n\n"
                                   f"支持格式: JPG, PNG, JPEG\n\n"
                                   f"当前默认处理方式: {DEFAULT_WORKFLOW}\n"
                                   "可用的处理方式:\n"
                                   "• 🎭 面部重绘\n"
                                   "• 🧹 去除背景杂物\n"
                                   "• ✂️ RC (多次迭代)\n"
                                   "• 💎 BF\n\n"
                                   "要更改默认处理方式，请修改代码中的 DEFAULT_WORKFLOW 变量")
